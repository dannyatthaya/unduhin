...
//!
//! NOTE — the `global_speed_limit_bps` setting is surfaced and
//! this bucket type is exposed, but the engine in this workspace does not
//! yet consume an external throttle. Until [`engine::download`] gains a
//! way to accept a per-byte gate (likely via a new field on
//! [`engine::DownloadOptions`]), the configured limit is informational
//! only. The bucket below is a working implementation so the wiring
//! lands in the right place; once the engine is updated, the queue
//! manager can feed each worker a clone of this `Arc<TokenBucket>`.
...
/// Refill-by-elapsed-time token bucket. Bytes per second.
#[derive(Debug)]
