...
impl DhtWorker {
    ...
    async fn framer(
        ...
        let reader = async {
            ...
            loop {
                // VENDORED PATCH (unduhin): on Windows, an ICMP error provoked
                // by a prior `send_to` (port/host/net unreachable, TTL expired)
                // is delivered as the *next* `recv_from`'s error. These are
                // per-datagram noise, not socket failures — but the original
                // `?` propagated them and killed the DHT framer, taking DHT down
                // for the whole session. Skip the whole family so DHT survives:
                //   WSAECONNRESET(10054)  WSAENETRESET(10052)  WSAENETUNREACH(10051)
                //   WSAEHOSTDOWN(10064)   WSAEHOSTUNREACH(10065)
                // Upstream librqbit (incl. 9.0-rc) never disables
                // SIO_UDP_CONNRESET, so this fork is the only fix. Re-sync on
                // librqbit bumps.
                let (size, addr) = match socket.recv_from(&mut buf).await {
                    ...
                    Err(e)
                        if e.kind() == std::io::ErrorKind::ConnectionReset
                            || matches!(
                                e.raw_os_error(),
                                Some(10054 | 10052 | 10051 | 10064 | 10065)
                            ) =>
                    {
                        debug!("ignoring transient UDP recv error (Windows ICMP): {e:#}");
                        continue;
                    }
                    Err(e) => return Err::<(), _>(e).context("error reading from UDP socket"),
                };
